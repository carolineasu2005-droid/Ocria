...
BENCHMARK_SESSION = os.environ.get(
    ...
    "qwen3_7_flash__r04_scene_art_v1",
)
...
# User-verified Alibaba Bailian Qwen3.7-Flash <=32K pricing snapshot.
...
PRICE_MAX_INPUT_TOKENS = 32_000
INPUT_PRICE_CNY_PER_1M = Decimal("0.2")
OUTPUT_PRICE_CNY_PER_1M = Decimal("0.8")
...
def build_summary(source_candidates: list[dict], rows: list[dict]) -> dict:
    ...
    return {
        ...
        "pricing_snapshot": {
            ...
            "note": (
                "Local estimate from the user-verified 2026-08-16 "
                "Qwen3.7-Flash <=32K price snapshot; actual billed cost "
                "may differ due to free quota, discounts, promotions, or later pricing."
            ),
        },
        "retry_policy": "none",
...
def main() -> None:
    api_key = require_env("R04_QWEN_API_KEY")
    base_url = require_env("R04_QWEN_BASE_URL")
